...
| `RES_TOO_HIGH` | `transcode_hevc_1080_profile.sh $vfo_input $vfo_output` |
| `ELSE` | `transcode_hevc_1080_profile.sh $vfo_input $vfo_output` |

## Runtime Behavior

- Scenario `CODEC_JUST_RIGHT RES_JUST_RIGHT` uses direct ffmpeg command execution.
- Scenario `RES_TOO_HIGH` uses action script `transcode_hevc_1080_profile.sh`.
- Scenario `ELSE` uses action script `transcode_hevc_1080_profile.sh`.
