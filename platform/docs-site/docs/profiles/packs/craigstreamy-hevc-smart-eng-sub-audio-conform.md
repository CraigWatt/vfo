...
This pack keeps the same subtitle-intent and HEVC posture as the existing craigstreamy preserve pack, but adds a stricter audio-delivery policy for DTS-family mezzanines.
...
- conforming DTS-family audio into open-source Dolby-aligned delivery codecs
...
- DTS-family cleanup without pretending we can author Atmos
...
| --- | --- |
| DTS mono / stereo | decode -> loudnorm -> AAC |
| DTS 3.0 / 4.0 / 5.0 / 5.1 | decode -> loudnorm -> E-AC-3 when available, else AC-3 |
| DTS 7.x / DTS:X style high-channel renders | decode/render -> downmix to 5.1 -> loudnorm -> E-AC-3 when available, else AC-3 |
...
flowchart LR
  ...
  C1 --> C2{DTS-family stream present?}:::gate
  D1 --> D2{DTS-family stream present?}:::gate
  E2 --> E3{DTS-family stream present?}:::gate

  C2 -->|Yes| C3[Conform DTS to AAC / E-AC-3 / AC-3 + loudnorm]:::stage
  C2 -->|No| C4[Preserve AAC / Dolby audio]:::stage
  D2 -->|Yes| D3[Conform DTS to AAC / E-AC-3 / AC-3 + loudnorm]:::stage
  D2 -->|No| D4[Preserve AAC / Dolby audio]:::stage
  E3 -->|Yes| E4[Conform DTS to AAC / E-AC-3 / AC-3 + loudnorm]:::stage
  ...
  Cq -->|No| Cmp4[Emit fragmented MP4]:::output
  Dq -->|Yes| Dmkv[Emit MKV]:::output
  Dq -->|No| Dmp4[Emit fragmented MP4]:::output
  Eq -->|Yes| Emkv[Emit MKV]:::output
  Eq -->|No| Emp4[Emit fragmented MP4]:::output
```
